...
TARGET_ISO=${2:-"`pwd`/{{inventory_hostname}}-{{virtual_machine['name']}}.iso"}
...
TMP_DOWNLOAD_DIR="{{git.directory}}/makeiso"
...
cd "$TMP_DISC_DIR"
rm -r '[BOOT]'
"$BIN_XORRISO" -as mkisofs -r -V "{{inventory_hostname}}-{{virtual_machine['name']}}.iso" -J -b isolinux.bin -c boot.cat -no-emul-boot -boot-load-size 4 -boot-info-table -input-charset utf-8 -isohybrid-mbr "$SCRIPT_DIR/custom/isohdpfx.bin" -eltorito-alt-boot -e boot/grub/efi.img -no-emul-boot -isohybrid-gpt-basdat -o "$TARGET_ISO" ./
